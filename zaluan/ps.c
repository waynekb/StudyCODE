...
static int getSystemStartTime(void){
    char buff[4096] = {0};
...
static int ps_line(int pid, int tid, char *namefilter)
{
    ...
    *ptr++ = '\0';           // and null-terminate name.
    ...
    int starttime = atoi(nexttok(&ptr)); // starttime
    ...
    /* if(!namefilter || !strncmp(name, namefilter, strlen(namefilter))) {
        ...
        printf(" %08x %08x %s %s", wchan, eip, state, name);
        if(display_flags&SHOW_TIME)
            printf(" (u:%d, s:%d, starttime:%d)", utime, stime, starttime);
    ...
    time_t starttime_time_t = getSystemStartTime() + starttime/100 ;
    localtime_r(&starttime_time_t, &starttime_tm);
    printf("\t%d-%d-%d %02d:%02d:%02d", starttime_tm.tm_year + 1900, starttime_tm.tm_mon + 1,  
        starttime_tm.tm_mday, starttime_tm.tm_hour, starttime_tm.tm_min, starttime_tm.tm_sec);
    printf("\t%-20s", cmdline[0] ? cmdline : name);
    printf("\n");
    return 0;
}
...
//gcc -o test ps.c  sched_policy.h sched_policy.c -lpthread
